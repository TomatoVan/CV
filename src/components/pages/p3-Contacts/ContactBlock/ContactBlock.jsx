	...
	let contactsBlock = contact.map((contact, index) => {
		return (
				<li  className={s.contactItem} key={index}>
					...
					<span className={s.contactIcon}><FontAwesomeIcon icon={contact.icon}/></span>
					{contact.text === 'To Github' ?
							<a className={s.contactLink} target='_blank' href='https://github.com/TomatoVan'>{contact.text}</a> :
